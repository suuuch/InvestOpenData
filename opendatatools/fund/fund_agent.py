...
class EastMoneyAgent(RestAgent):
    ...
    def get_fundlist_by_type(self, fund_type):
        if fund_type not in fund_types:
            return None, '不正确的基金类型，请通过get_fund_type查询'
        type_param = fund_types[fund_type]

        page_no = 1
        page_size = 1000
        df_result = []
        while True:
            df, msg = self._get_fundlist_by_type_page(fund_type, page_no, page_size)
            if df is not None:
                df_result.append(df)
